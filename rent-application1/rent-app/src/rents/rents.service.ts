...
export class RentsService {
    ...
    async getAllRentsForDashboard(user: User): Promise<Rent[]> {
        this.logger.log(`Fetching rents for dashboard for user with id: "${user.id}"`);
        try {
            const rents = await this.rentsRepository.createQueryBuilder('rent')
                .where('rent.userId = :userId', { userId: user.id })
                .getMany();

            if (!rents || rents.length === 0) {
                throw new NotFoundException(`No rents found for user with id: "${user.id}"`);
            }

            return rents;
        } catch (error) {
            this.logger.error('Error fetching rents for dashboard:', error);
            throw new HttpException('Failed to get rents for dashboard', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
